...
async function initExamEngine() {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}: Could not load test.json. Check file placement.`);
    
    testData = await res.json();

    if (!testData.sections || !Array.isArray(testData.sections)) {
      throw new Error("Invalid test.json format: 'sections' array missing.");
    }
...
function loadQuestion(index) {
  ...
  let questionHTML = `<div class="mb-3 text-slate-900 dark:text-slate-100">${q.question || ''}</div>`;
  if (q.image) {
    questionHTML += `
...
function openImageModal(src) {
  ...
  if (!modal) {
    ...
    modal.className = 'fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 cursor-zoom-out';
    modal.onclick = () => modal.remove();
    document.body.appendChild(modal);
  }
  modal.innerHTML = `<div class="relative max-w-4xl max-h-[90vh] bg-white p-2 rounded-2xl"><img src="${src}" class="max-h-[85vh] max-w-full object-contain rounded-xl" /></div>`;
}